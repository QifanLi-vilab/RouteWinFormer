...
def paired_dehazing_paths_from_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    ...
    for idx in range(len(input_paths)):
        input_path = input_paths[idx]
        input_basename, input_ext = osp.splitext(osp.basename(input_path))

        gt_basename = input_basename.split('_')[0]
        _, gt_ext = osp.splitext(osp.basename(gt_paths[0]))

        gt_name = f'{filename_tmpl.format(gt_basename)}{gt_ext}'
        input_path = osp.join(input_folder, input_path)
        assert gt_name in gt_paths, (f'{gt_name} is not in '
                                        f'{input_key}_paths.')
        gt_path = osp.join(gt_folder, gt_basename + gt_ext)
        paths.append(
            dict([(f'{input_key}_path', input_path),
                  (f'{gt_key}_path', gt_path)]))
    return paths
